...
//
// IMPORTANT FIX:
//   Row height is now **constant per page type** (first/other). We compute
//   a fixed row height using the page’s available height divided by the
//   *maximum* rows for that page (33 on first, 43 on others). If a page has
//   fewer lines, we **do not stretch rows**—we leave the extra space empty.
//
// Usage:
//   await printCertifiedStatement(lines, accountInfo)
...
export async function printCertifiedStatement(
  ...
  // Pagination settings (maximum rows per page)
  const MAX_ROWS_FIRST_PAGE = 33;
  const MAX_ROWS_OTHER_PAGES = 43;
  ...
  while (idx < lines.length || pageIndex === 0) {
    ...
    // FIX: use a **fixed** row height based on the *maximum* rows for this page type
    const maxRowsThisPage = isFirst ? MAX_ROWS_FIRST_PAGE : MAX_ROWS_OTHER_PAGES;
    const rowH = maxRowsThisPage > 0 ? usableH / maxRowsThisPage : 0;

    // Only draw as many rows as we have data for, but never change row height
    const rowsRemaining = lines.length - idx;
    const rowsToDraw = Math.min(maxRowsThisPage, Math.max(0, rowsRemaining));

    // Body rows (constant row height, no stretching)
    let y = tableHeaderY + HEADER_H;
    for (let n = 0; n < rowsToDraw; n++, idx++) {
      const L = lines[idx];
