...
import LyPayRowActions, {
  type LyPayActionName,
} from "../components/LyPayRowActions";
import LyPayStatusBadge from "../components/LyPayStatusBadge";
import { formatLyPayAmount, formatLyPayDateTime } from "../formatters";
import { canApproveLyPayTransfer } from "../permissions";
import {
  approveLyPayTransfer,
  getLyPayTransferById,
  refreshLyPayTransferStatus,
} from "../services";
...
export default function LyPayTransferDetailsPage() {
  const t = useTranslations("lyPayTransfer");
  const statusT = useTranslations("lyPayTransfer.statuses");
  const locale = useLocale();
  const params = useParams<{ id: string }>();
  const transferId = Number(params?.id);

  const [transfer, setTransfer] = useState<LyPayTransfer | null>(null);
  const [loading, setLoading] = useState(true);
  const [loadError, setLoadError] = useState("");
  const [canApprove, setCanApprove] = useState(false);
  const [activeAction, setActiveAction] =
    useState<Exclude<LyPayActionName, null> | null>(null);
  const [approvalOpen, setApprovalOpen] = useState(false);
  const [notice, setNotice] = useState<NoticeState>(EMPTY_NOTICE);

  useEffect(() => {
    setCanApprove(canApproveLyPayTransfer());
  }, []);
  ...
  useEffect(() => {
    ...
    const loadTransfer = async () => {
      ...
      try {
        ...
        const result = await getLyPayTransferById(transferId);
  ...
  const showActionResult = (updated: LyPayTransfer) => {
  ...
  const approveTransfer = async () => {
    ...
    try {
      const updated = await approveLyPayTransfer(transfer.id);
      setTransfer(updated);
  ...
  const refreshStatus = async () => {
    ...
    try {
      const updated = await refreshLyPayTransferStatus(transfer.id);
      setTransfer(updated);
      showActionResult(updated);
    } catch (error) {
      setNotice({
      ...
      <div className="rounded-xl bg-white p-3 shadow-sm sm:p-5">
        <div className="flex flex-wrap items-center gap-3 rounded-md bg-info-dark p-4 text-white">
          ...
          <div className="flex w-full flex-wrap items-center justify-between gap-2 sm:w-auto sm:justify-end ltr:sm:ml-auto rtl:sm:mr-auto">
            <LyPayStatusBadge status={transfer.status} />
            {hasAvailableAction && (
              <LyPayRowActions
                transfer={transfer}
        ...
        <section className="mt-6">
          ...
          <dl className="grid gap-3 md:grid-cols-2">
            ...
            <DetailItem
              ...
              value={`${formatLyPayAmount(transfer.amount, locale)} ${transfer.currency}`}
            />
        ...
        <section className="mt-6">
          ...
          <dl className="grid gap-3 md:grid-cols-2">
            ...
            <DetailItem
              ...
              value={formatLyPayDateTime(transfer.createdAt, locale)}
            />
            <DetailItem label={t("details.approvedBy")} value={transfer.approvedByName} />
            <DetailItem
              label={t("details.validatedAt")}
              value={formatLyPayDateTime(transfer.validatedAt, locale)}
            />
            <DetailItem
              label={t("details.executedAt")}
              value={formatLyPayDateTime(transfer.executedAt, locale)}
            />
            <DetailItem
              label={t("details.completedAt")}
              value={formatLyPayDateTime(transfer.completedAt, locale)}
            />
          </dl>
        </section>
