...
import {
  createLyPayTransfer,
  getLyPayAccounts,
  getLyPayInstitutions,
  validateLyPayTransfer,
} from "../services";
import type {
  LyPayAccount,
  LyPayDeviceInfo,
...
type LyPayFormProps = {
  onCreated: (transfer: LyPayTransfer) => void;
};
...
function FormikIbanInput({
  name,
  label,
  placeholder,
  disabled = false,
  onValueChange,
}: FormikIbanInputProps) {
  const [field, meta, helpers] = useField<string>(name);
  const hasError = Boolean(meta.touched && meta.error);
  ...
  return (
    <div className="mb-4 w-full">
      ...
      <input
        ...
        onBlur={() => helpers.setTouched(true)}
        onChange={(event) => {
          const value = normalizeLibyanIban(event.target.value);
          void helpers.setValue(value);
          onValueChange?.(value);
        }}
        className={`w-full rounded-md border p-2 font-mono uppercase text-black focus:outline-none focus:ring disabled:cursor-not-allowed disabled:bg-gray-200 disabled:text-gray-500 ${
          hasError
            ? "border-red-500 focus:ring-red-500"
            : "border-gray-300 focus:ring-blue-500"
        }`}
...
const getOptionalLocation = (): Promise<LyPayDeviceInfo | undefined> => {
...
export default function LyPayForm({ onCreated }: LyPayFormProps) {
  const t = useTranslations("lyPayTransfer");
  const locale = useLocale();

  const [accounts, setAccounts] = useState<LyPayAccount[]>([]);
  const [institutions, setInstitutions] = useState<LyPayInstitution[]>([]);
  ...
  const [validation, setValidation] =
    useState<LyPayValidationResponse | null>(null);
    ...
    const loadOptions = async () => {
      ...
      try {
        const [accountValues, institutionValues, beneficiaryResult] =
          await Promise.all([
            getLyPayAccounts(),
            getLyPayInstitutions(locale === "ar" ? "ar" : "en"),
            getBeneficiariesForRail("lyPay")
  ...
  const resolveBeneficiaryInstitution = useCallback(
    (beneficiary: BeneficiaryResponse): LyPayInstitution | undefined =>
      institutions.find(
    ...
    () =>
      Yup.object({
        ...
        toAccount: Yup.string()
          .trim()
          .required(t("validation.toRequired"))
          .test("libyan-iban", t("validation.toFormat"), (value) =>
            value ? isValidLibyanIban(normalizeLibyanIban(value)) : true
          )
          .test(
            "institution-matches-iban",
            t("validation.toBankMismatch"),
            function (value) {
              const institutionId = String(
                this.parent.toInstitutionId ?? ""
              );
              if (!value || !institutionId) return true;
              return (
                normalizeLibyanIban(value).slice(4, 7) === institutionId
              );
            }
          ),
        amount: Yup.number()
  ...
  const initialValues: LyPayFormValues = {
    beneficiaryId: "",
    fromAccount: "",
    toInstitutionId: "",
    toAccount: "",
  ...
  const createTransfer = async () => {
    ...
    setIsCreating(true);
    try {
      const transfer = await createLyPayTransfer({
  ...
  return (
    <div className="rounded-xl bg-white p-3 shadow-sm sm:p-5">
      <div className="flex items-center gap-3 rounded-md bg-info-dark p-4 text-white">
        <BackButton fallbackPath="/transfers/lypay" isEditing />
        <div>
      ...
      >
        {({ values, setFieldValue, setFieldTouched }) => {
          ...
          const applyBeneficiary = async (beneficiaryId: string) => {
            ...
            // Preserve bank-before-account ordering for dependent IBAN validation.
            await setFieldValue(
              "toInstitutionId",
            ...
            await setFieldValue(
              "toAccount",
              normalizeLibyanIban(beneficiary.accountNumber),
              false
            );
            await setFieldTouched("toAccount", false, false);
          };
