...
import {
  canApproveLyPayTransfer,
  canCreateLyPayTransfer,
} from "./permissions";
import {
...
type ActiveAction = {
  transferId: number;
  name: Exclude<LyPayActionName, null>;
} | null;
...
export default function LyPayTransfersPage() {
  ...
  useEffect(() => {
    setCanCreate(canCreateLyPayTransfer());
  ...
  const fetchTransfers = useCallback(async () => {
    setLoading(true);
    try {
      const result = await getLyPayTransfers(
        currentPage,
        PAGE_SIZE,
        searchTerm
      );
  ...
  const updateTransfer = (updated: LyPayTransfer) => {
  ...
  const approveTransfer = async () => {
    ...
    try {
      const updated = await approveLyPayTransfer(transferId);
  ...
  const refreshTransfer = async (transfer: LyPayTransfer) => {
  ...
  const columns: DataGridColumn[] = [
      { key: "referenceNo", label: t("columns.reference") },
      {
        ...
        renderCell: (row: LyPayTransfer) =>
          formatLyPayDateTime(row.createdAt, locale),
      },
      ...
      { key: "toAccountName", label: t("columns.recipientName") },
      {
        ...
        renderCell: (row: LyPayTransfer) =>
          `${formatLyPayAmount(row.amount, locale)} ${row.currency}`,
      },
      { key: "createdByName", label: t("columns.createdBy") },
      {
        ...
        renderCell: (row: LyPayTransfer) => (
          <LyPayStatusBadge status={row.status} />
        ),
      },
      {
        key: "actions",
        ...
        renderCell: (row: LyPayTransfer) => (
          <LyPayRowActions
            ...
            onView={() =>
              router.push(`/${locale}/transfers/lypay/${row.id}`)
            }
  ...
  return (
    <div className="p-2 sm:p-4">
      <CrudDataGrid
        ...
        onAddClick={() => router.push(`/${locale}/transfers/lypay/add`)}
        showActions={false}
        loading={loading}
        canEdit
      />
